...
- `MapSection` uses one Google Maps JavaScript map with the two real locations and no route; both supplied destination links remain visible beside it. The Google Cloud project `zicatela` has a granted `250` map-loads-per-day quota, at most `7,750` loads in a 31-day month, and the browser key is restricted to the approved local and Casa Zii domains. The current billing account is shared, so this bound is not an absolute `$0` guarantee: `zicatela` must become the only Dynamic Maps project on a dedicated billing account before that guarantee can be made.
